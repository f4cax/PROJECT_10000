  ...
  const fetchStockData = useCallback(async (showLoading = true) => {
    try {
      ...
      if (API_KEY && API_KEY !== 'demo') {
        ...
        try {
          console.log('Загружаем реальные данные от Alpha Vantage для всех акций...');
          const realData = {};
          let successCount = 0;
          let failCount = 0;
          
          // Загружаем данные для всех акций с задержками
          for (let i = 0; i < popularStocks.length; i++) {
            const stock = popularStocks[i];
            try {
              console.log(`Загружаем данные для ${stock.symbol} (${i + 1}/${popularStocks.length})...`);
              ...
              realData[stock.symbol] = {
                ...
                high52w: data.high ? data.high * 1.2 : data.price * 1.2,
                low52w: data.low ? data.low * 0.8 : data.price * 0.8
              };
              
              successCount++;
              console.log(`✅ Успешно загружено: ${stock.symbol}`);
              
              // Задержка между запросами для соблюдения лимитов API
              // Alpha Vantage рекомендует не более 5 запросов в минуту для бесплатного плана
              if (i < popularStocks.length - 1) {
                console.log('Ожидание 12 секунд перед следующим запросом...');
                await new Promise(resolve => setTimeout(resolve, 12000)); // 12 секунд между запросами
              }
              
            } catch (err) {
              console.error(`❌ Ошибка загрузки ${stock.symbol}:`, err.message);
              failCount++;
              
              // Если это ошибка лимита API, прерываем загрузку
              if (err.message.includes('API limit') || err.message.includes('calls per minute')) {
                console.log('🚫 Достигнут лимит API, прерываем загрузку');
                break;
              }
              
              // Для других ошибок продолжаем со следующей акцией
          ...
          // Если получили хотя бы одну реальную акцию, дополняем остальные демо-данными
          if (successCount > 0) {
            ...
            if (successCount === popularStocks.length) {
              setError(null);
              console.log('🎉 Все акции загружены с реальными данными!');
            } else {
              setError(`Смешанный режим: ${successCount} акций с реальными данными, ${failCount} с демо-данными`);
              console.log(`📊 Смешанный режим: ${successCount} реальных, ${failCount} демо`);
            }
          } else {
      ...
      <div className="max-w-6xl mx-auto space-y-8 fade-in">
        ...
        <div className="card text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-500">
            Загружаем реальные котировки для всех акций.<br/>
            Это может занять 1-2 минуты из-за лимитов API...
          </p>
        </div>
      </div>
    );
